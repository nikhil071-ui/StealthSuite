...
export default function TranslatorPage() {
  ...
  const languages = ["Spanish", "French", "German", "Italian", "Japanese", "Chinese", "Hindi", "Russian"];
    ...
    <div className="min-h-screen bg-[#030305] text-gray-100 font-sans flex flex-col relative">
      ...
      <nav className="border-b border-white/5 bg-black/20 backdrop-blur-xl sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
           ...
           <div className="font-black text-xl tracking-tight text-teal-500">TRANSLATOR</div>
      ...
      <div className="flex-1 max-w-6xl mx-auto px-4 py-8 grid lg:grid-cols-2 gap-8 w-full">
        ...
        <div className="flex flex-col gap-4">
           <div className="bg-[#0A0A0F]/80 backdrop-blur-sm border border-white/10 rounded-2xl p-1 shadow-2xl h-full flex flex-col">
              <div className="flex justify-between items-center p-3 border-b border-white/5">
                <span className="text-xs font-bold text-gray-500 uppercase px-2">Input Text</span>
              ...
              <textarea
                 ...
                 placeholder="Paste text to translate natively..."
                 value={input}
              ...
              <div className="p-2 border-t border-white/5 flex flex-wrap gap-2">
                 {languages.map((l) => (
                   <button
                     key={l}
                     onClick={() => setLang(l)}
                     className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase transition-all ${lang === l ? "bg-teal-500/20 text-teal-400 border border-teal-500/30" : "text-gray-500 hover:text-gray-300 hover:bg-white/5"}`}
                   >
                     {l}
                   </button>
                 ))}
        ...
        <div className="flex flex-col gap-4 relative">
           ...
           <div className="absolute top-1/2 -left-3 -translate-x-1/2 -translate-y-1/2 z-10 hidden lg:block pointer-events-none">
              <div className="w-8 h-8 bg-teal-600/20 rounded-full flex items-center justify-center border border-teal-500/50 text-teal-400">➜</div>
           </div>
           ...
           <button 
                ...
                className="lg:hidden w-full py-4 bg-teal-600 rounded-xl font-bold text-white disabled:opacity-50"
           >
                {loading ? "Translating..." : "Translate Text"}
           </button>

           <div className={`bg-[#0A0A0F]/80 backdrop-blur-sm border rounded-2xl p-1 shadow-2xl h-full flex flex-col relative transition-all ${output ? 'border-teal-500/30' : 'border-white/10'}`}>
              <div className="flex justify-between items-center p-3 border-b border-white/5">
                 <span className="text-xs font-bold text-teal-400 uppercase px-2">Native Translation</span>
                 {output && (
        ...
        <div className="hidden lg:flex justify-center col-span-2 mt-4">
             <button
               onClick={handleProcess}
               disabled={loading || !input}
               className="group relative px-12 py-4 rounded-full font-bold text-lg transition-all duration-300 transform hover:scale-105 active:scale-95 disabled:opacity-50 bg-teal-600 text-white shadow-[0_0_40px_-10px_rgba(20,184,166,0.5)] border border-teal-400/30"
             >
