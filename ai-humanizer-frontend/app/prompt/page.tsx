  ...
  const handleProcess = async () => {
    ...
    try {
      const res = await fetch("http://localhost:8000/api/prompt", {
        ...
        body: JSON.stringify({ text: input, goal: goal }),
      });
      const data = await res.json();
      setOutput(data.refined_prompt);
    } catch (error) {
    ...
    <div className="min-h-screen bg-[#030305] text-gray-100 font-sans flex flex-col relative">
      ...
      <nav className="border-b border-white/5 bg-black/20 backdrop-blur-xl sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
           ...
           <div className="font-black text-xl tracking-tight text-pink-500">PROMPT PERFECTOR</div>
      ...
      <div className="flex-1 max-w-6xl mx-auto px-4 py-8 grid lg:grid-cols-2 gap-8 w-full">
        ...
        <div className="flex flex-col gap-4">
           <div className="bg-[#0A0A0F]/80 backdrop-blur-sm border border-white/10 rounded-2xl p-1 shadow-2xl h-full flex flex-col">
              <div className="flex justify-between items-center p-3 border-b border-white/5">
                <span className="text-xs font-bold text-pink-400 uppercase px-2">Lazy Input</span>
              ...
              <textarea
                 ...
                 placeholder='e.g. "Write me an essay about dogs"...'
                 value={input}
              ...
              <div className="p-2 border-t border-white/5 flex flex-wrap gap-2">
                 {["general", "creative", "coding", "academic"].map((g) => (
                   <button
                     key={g}
                     onClick={() => setGoal(g)}
                     className={`px-4 py-1.5 rounded-lg text-xs font-bold uppercase transition-all ${goal === g ? "bg-pink-500/20 text-pink-400 border border-pink-500/30" : "text-gray-500 hover:text-gray-300 hover:bg-white/5"}`}
                   >
                     {g}
                   </button>
                 ))}
        ...
        <div className="flex flex-col gap-4 relative">
           ...
           <div className="absolute top-1/2 -left-3 -translate-x-1/2 -translate-y-1/2 z-10 hidden lg:block pointer-events-none">
              <div className="w-8 h-8 bg-pink-600/20 rounded-full flex items-center justify-center border border-pink-500/50 text-pink-400">➜</div>
           </div>
           ...
           <button 
                ...
                className="lg:hidden w-full py-4 bg-pink-600 rounded-xl font-bold text-white disabled:opacity-50"
           >
                {loading ? "Refining..." : "Perfect My Prompt"}
           </button>

           <div className={`bg-[#0A0A0F]/80 backdrop-blur-sm border rounded-2xl p-1 shadow-2xl h-full flex flex-col relative transition-all ${output ? 'border-pink-500/30' : 'border-white/10'}`}>
              <div className="flex justify-between items-center p-3 border-b border-white/5">
                 <span className="text-xs font-bold text-pink-400 uppercase px-2">God-Tier Prompt</span>
                 {output && (
        ...
        <div className="hidden lg:flex justify-center col-span-2 mt-4">
             <button
               onClick={handleProcess}
               disabled={loading || !input}
               className="group relative px-12 py-4 rounded-full font-bold text-lg transition-all duration-300 transform hover:scale-105 active:scale-95 disabled:opacity-50 bg-pink-600 text-white shadow-[0_0_40px_-10px_rgba(219,39,119,0.5)] border border-pink-400/30"
             >
